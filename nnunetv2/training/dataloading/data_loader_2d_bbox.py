...
class nnUNetDataLoader2DBBOX(nnUNetDataLoaderBase):
    ...
    def get_bbox(self, seg: np.ndarray):
        uniques = np.unique(seg)[1:]
        cls = np.random.choice(uniques)
        ...
        bbox_lbs = [np.random.randint(bbox[0]-self.patch_size[0]+size[0], bbox[0]+1),
                    np.random.randint(bbox[2]-self.patch_size[1]+size[1], bbox[2]+1)]
        bbox_ubs = [bbox_lbs[i] + self.patch_size[i] for i in range(dim)]
        bbox_mask = np.zeros_like(seg[0])
        ...
        return bbox_lbs, bbox_ubs, seg_inst[None].astype(seg.dtype), bbox_mask[None].astype(seg.dtype)
    ...
    def generate_train_batch(self):
        ...
        bbox_mask_all = np.zeros(self.seg_shape, dtype=np.int16)
        case_properties = []
        ...
        for j, current_key in enumerate(selected_keys):
            ...
            shape = data.shape[1:]
            dim = len(shape)
            bbox_lbs, bbox_ubs, seg, bbox_mask = self.get_bbox(seg)
            ...
            seg = seg[this_slice]
            bbox_mask = bbox_mask[this_slice]

            padding = [(-min(0, bbox_lbs[i]), max(bbox_ubs[i] - shape[i], 0)) for i in range(dim)]
            data_all[j] = np.pad(data, ((0, 0), *padding), 'constant', constant_values=0)
            seg_all[j] = np.pad(seg, ((0, 0), *padding), 'constant', constant_values=-1)
            bbox_mask_all[j] = np.pad(bbox_mask, ((0, 0), *padding), 'constant', constant_values=0)

        return {'data': data_all, 'seg': seg_all, 'bbox_mask': bbox_mask_all, 'properties': case_properties, 'keys': selected_keys}
